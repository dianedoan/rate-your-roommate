...
const CreateReviewPage = ({ userId, sortKey, userCity }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${
          config.apiBaseUrl
        }/write-review?UserId=${userId}&SortKey=${encodeURIComponent(sortKey)}`,
        {
          method: "POST",
  ...
  const validateForm = () => {
    const allRatingsAnswered = Object.values(ratings).every(
      (rating) => rating > 0
    );
    const allYesNoAnswered = Object.values(yesNoQuestions).every(
      (answer) => answer !== ""
    );
  ...
  return (
    <div className="create-review-content">
      <div className="create-review-header">
        {reviewProfile ? (
          <>
            <div className="create-review-name">
              Rate: {reviewProfile.FirstName} {reviewProfile.LastName}
            </div>
            <div className="occupation-location-container">
              <div className="create-review-occupation">
                {reviewProfile.Occupation}
              </div>
              <div className="create-review-location">
                {reviewProfile.City}, {reviewProfile.State}
              </div>
            </div>
          </>
        ) : (
      ...
      <form onSubmit={handleSubmit} className="review-form">
        <div className="questions-section">
          {[
            ...
            return (
              <div key={category} className="question-card rating-category">
                <label className="question-label" htmlFor={categoryLabel}>
                  {categoryLabel}
                </label>
                <div className="stars-container">
                  {renderStars(
                    category,
                    labelMapping[category][0],
                    labelMapping[category][1]
                  )}
                </div>
              </div>
            );
          ...
          <div className="question-card open-ended-section">
            <label className="question-label" htmlFor="openEnded">
              What do you want other users to know about this roommate?
            </label>
            <textarea
          ...
          <div className="create-review-button-container">
            <button
              type="submit"
              className="primary-btn submit-review-btn"
              disabled={loading}
            >
